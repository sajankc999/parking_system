...
from Parking.models import ParkingDetails,ParkingSpace,Vehicle_info
...
def ParkingSpaceCreateView(request):
    ...
    if request.method == "POST":
        ...
        if number < 0 or number > 10:
            context = {
                "error": "number should be less then 10 and greater than 0."}
...
def VehicleDetailsAdd(request):
    ...
    if request.method == "POST":
        ...
        Vehicle_info.objects.create(
            type=data.get("type"), plate_no=data.get("plate_no")
        )
...
def parking_details(request):
    ...
    if request.method == "POST":
        ...
        if parking := ParkingDetails.objects.filter(
